...
# Initialize PID so cleanup doesn't fail on early exit
BACKEND_PID=""
...
echo -e "  API port:  ${BOLD}${BACKEND_PORT}${NC}"
echo ""
...
# Wait for the backend process to exit, but ignore errexit for this wait
set +e
wait "$BACKEND_PID"
EXIT_CODE=$?
set -e
warn "Backend process exited (exit code: ${EXIT_CODE}), shutting down..."
cleanup
