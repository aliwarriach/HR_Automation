...
from app.routers import (
    candidate_matches_router,
    health_router,
    jobs_router,
    resume_detail_router,
    resumes_router,
)
...
async def lifespan(app: FastAPI):
    ...
    migrate_legacy_keys()
    yield
    # Shutdown - wrap in try-except to ensure the resource is released
    try:
        await db.close()
...
app = FastAPI(
    ...
    description="AI-powered ATS resume checker and scorer",
    version=__version__,
    lifespan=lifespan,
)
...
app.include_router(resume_detail_router, prefix="/api/v1")
