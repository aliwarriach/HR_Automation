...
# Old (pre-prefix) table name -> (new prefixed name, a column that must be
# present to treat it as *our* legacy table rather than some other
# application's unrelated table of the same old name). This lets a self-hosted
# upgrade rename its own tables in place (no data loss) while a foreign table
# that merely happens to share an old name (e.g. a host app's own `resumes`
# table when this database file is shared) is never touched.
_LEGACY_TABLE_RENAMES: tuple[tuple[str, str, str], ...] = (
    ("resumes", "resume_matcher_documents", "resume_id"),
    ("jobs", "resume_matcher_jobs", "job_id"),
    ("improvements", "resume_matcher_improvements", "request_id"),
    ("applications", "resume_matcher_applications", "application_id"),
    ("api_keys", "resume_matcher_api_keys", "ciphertext"),
)
...
def _migrate_legacy_table_names(engine: Engine) -> None:
    """Rename this app's own pre-prefix tables in place, if found.

    Safe to run against a database file owned by another application: a
    rename only happens when the old-named table has the signature column
    unique to Resume Matcher's own schema. Anything else is left untouched.
    """
    with engine.begin() as conn:
        for old_name, new_name, signature_column in _LEGACY_TABLE_RENAMES:
            ...
            if signature_column in columns:
                conn.exec_driver_sql(f"ALTER TABLE {old_name} RENAME TO {new_name}")
