...
RESUME_DETAIL_EXTRACTION_PROMPT = """Extract candidate summary details from this resume as JSON. Output ONLY the JSON object, no other text.

Example output format:
{schema}

Field rules:
- "candidate_name": full name of the candidate; "" if not found.
- "university": the institution of the candidate's most recent/current education entry; "" if none.
- "education_status": one of:
  - "Graduate" if the candidate has completed a degree (graduation date in the past, or explicitly stated as graduated/completed).
  - "<Nth> Semester" (e.g. "5th Semester") if currently enrolled and a semester/year can be inferred from dates or text.
  - "Undergraduate" if not currently enrolled in any university and has not graduated.
- "cgpa": the CGPA/GPA exactly as written (e.g. "3.8/4.0"); "N/A" if not mentioned anywhere.
- "current_city": the candidate's current city from contact/location info; "" if not found.
- "total_experience_years": total professional work experience in years, computed from all workExperience date ranges (not education or projects). Use decimals for partial years (e.g. 0.5 for 6 months, 1.5 for a year and a half). Use 0 if no experience.
- "headline_role": the job title of the most recent/current work experience entry; "" if none.
- "headline_company": the company of the most recent/current work experience entry; "" if none.
