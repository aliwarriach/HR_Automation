...
class Settings(BaseSettings):
    ...
    # Absolute path override to share a SQLite file with another application
    # (e.g. an existing hr_app.db). Resume Matcher's own tables are created
    # with a `resume_matcher_` prefix so they can't collide with tables that
    # file already owns. Leave unset to use the default `data/resume_matcher.db`.
    external_db_path: Path | None = None
    ...
    def sqlite_path(self) -> Path:
        ...
        if self.external_db_path is not None:
            return self.external_db_path
