...
async def isolated_db(tmp_path, monkeypatch):
    ...
    of a MagicMock, so persistence and CRUD are actually exercised — without
    touching the developer's real database. A temp **file** (not ``:memory:``)
    is required: SQLite's connection pool gives each connection its own
    in-memory DB, so the async + sync engines would not share state.
    """
